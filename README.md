...
PermissionError: [Errno 13] <br /> Permission denied: '/dev/serial0' | you need to give the permission to acces the Serial Port to your current user <br /> You may need to add your user (pi) to the dialout group with `sudo usermod -a -G dialout pi`, or make sure that `/dev/serial0` has read/write permissions by calling `sudo chmod 666 /dev/serial0`.
